...
_CK2EXTSTBD_() {
	...
	( [[ -f "$EXTSTTD/buildAPKs/.conf/VERSIONID" ]] && ESVERSIONID="$(head -n 1 $EXTSTTD/buildAPKs/.conf/VERSIONID)" && printf "%s" "$FILENDSTRING found file $EXTSTTD/buildAPKs/.conf/VERSIONID : $ESVERSIONID : " && export EXTSTBD=0 ) || ( printf "%s" "$FILENDSTRING did not find file $EXTSTTD/buildAPKs/.conf/VERSIONID : " && export EXTSTBD=1 ) 
