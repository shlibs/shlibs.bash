...
_MAINCALIB_() { # print information to terminal from file .conf/LIBAUTH
	...
	then
		...
		if [[ "$PV1" == "false" ]]
		then
			printf "\\e[7;38;5;122m%s\\n\\n%s\\e[0m\\n" "File ~/"${RDR##*/}"/.conf/GAUTH has information about enabling OAuth authentication to increase the rate limit for authenticated Github users." "Also, file ~/"${RDR##*/}"/.conf/LIBAUTH has information regarding the integration of artifacts and libraries into compilations.  The functionality of this option is being enhanced.  To improve this automation see https://github.com/BuildAPKs/buildAPKs/issues and pulls..."
			printf "\\n\\e[7;38;5;122m%s\\e[0m\\n" "Running \$(cat "$RDR"/.conf/GAUTH "$RDR"/.conf/LIBAUTH):"
			printf "%s\\e[0m\\n" "$(cat "$RDR"/.conf/GAUTH "$RDR"/.conf/LIBAUTH):"
			CALIBCT="$(( CALIBCT - 3 ))"
			printf "\\e[7;38;5;122m%s\\e[0m\\n" "${CALIBCT#-} showings of files ~/"${RDR##*/}"/.conf/{GAUTH,LIBAUTH} remaining; Continuing..."
		fi
	fi
}
